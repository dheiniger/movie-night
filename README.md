...
* [`src/movie_picker/`](src/movie_picker/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/movie_picker/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `movie-picker` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/movie_picker/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns movie-picker.example
  (:require [movie-picker.config :as config])
